# == Defined type: heka::plugin::input::udpinput
#
# This defined type is for creating Heka udpinput plugin TOML files.
# See the Heka documentation for more info on udpinput plguins:
#
# http://hekad.readthedocs.org/en/latest/config/inputs/udp.html
#
...
# @param plugin_file_template String; the ERB template to use to generate the TOML plugin file; defaults to `heka/plugins/inputs/heka_udpinput_plugin.toml.erb`
# @param refresh_heka_service Bool; whether to refresh the Heka daemon when a config file is created or modified.
# See this page for more details on the plugin-specific options: http://hekad.readthedocs.org/en/latest/config/inputs/udp.html
